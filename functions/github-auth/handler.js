...
module.exports.handler = function(event, context, cb) {
  ...
  Request.auth({
    ...
    s3bucket.createBucket(function(err) {
      ...
      var params = {Key: response.body.user_id, Body: JSON.stringify(response.body)};
      s3bucket.upload(params, function(err, data) {
        ...
        if (err) {
          ...
          console.log("Successfully uploaded data to "+process.env.S3_BUCKET_NAME_GITRDONE+"/"+response.body.user_id);
        }
        ...
        return context.done(null, {
          location: "https://github.com/login/oauth/authorize?" + qs.stringify(params)
        });
      });
    });
  });
